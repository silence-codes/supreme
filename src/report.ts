...
import { ScanResult, UnifiedIssue } from './trivy';
...
export function generateReportHtml(results: ScanResult[], webview: vscode.Webview): string {
    const style = `
        ...
        * {
        ...
        .issue-title {
            ...
            color: #fff;
            margin-top: 8px;
            word-break: break-word; /* Crucial for long package names */
            line-height: 1.3;
        }
        ...
        .meta-value {
            font-size: 0.9rem;
            color: #eee;
            word-break: break-all; /* Crucial for long URLs/Hashes */
    ...
    results.forEach(target => {
        ...
        target.Issues.forEach(issue => {
            ...
            const fixedInHtml = issue.FixedVersion
                ? `<div class="meta-item"><span class="meta-label">Fixed In</span><span class="meta-value" style="color:#2ecc71">${issue.FixedVersion}</span></div>`
                : '';

            const linkHtml = issue.Url
                ? `<div class="meta-item"><span class="meta-label">Reference</span><a href="${issue.Url}" class="meta-value">${issue.Url}</a></div>`
                : '';

            const depPathHtml = issue.PkgPath
                ? `<div class="meta-item" style="grid-column: 1 / -1;"><span class="meta-label">Dependency Chain</span><span class="meta-value" style="color:#a29bfe; word-break:break-all;">${issue.PkgPath}</span></div>`
                : '';
